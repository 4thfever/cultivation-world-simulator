...
*   **性能优化与避坑 (Gotchas)**:
    ...
    *   地图区域名（`MapLayer` / `utils/mapLabels.ts`）默认应采用“碰撞避让”而非“重叠即隐藏”。避让策略必须兼容 `static/locales/registry.json` 中当前启用语言（如 `zh-CN / zh-TW / en-US / vi-VN / ja-JP / fr-FR`）：紧凑脚本（中文、日文）与拉丁脚本（英文、越南文、法文等）分别使用各自的文字尺寸估算与换行策略。
