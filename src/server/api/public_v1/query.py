from __future__ import annotations

from typing import Callable, Any

from fastapi import APIRouter, Query

from src.server.services.public_api_contract import ok_response


def create_public_query_router(
    *,
    query_service: Any | None = None,
    build_runtime_status: Callable[[], dict] | None = None,
    build_world_state: Callable[[], dict] | None = None,
    build_world_map: Callable[[], dict] | None = None,
    build_map_presets: Callable[..., dict] | None = None,
    build_current_run: Callable[[], dict] | None = None,
    build_events_page: Callable[..., dict] | None = None,
    build_rankings: Callable[[], dict] | None = None,
    build_sect_relations: Callable[[], dict] | None = None,
    build_game_data: Callable[[], dict] | None = None,
    build_avatar_adjust_options: Callable[[], dict] | None = None,
    build_avatar_meta: Callable[[], dict] | None = None,
    build_avatar_list: Callable[[], dict] | None = None,
    build_phenomena: Callable[[], dict] | None = None,
    build_sect_territories: Callable[[], dict] | None = None,
    build_mortal_overview: Callable[[], dict] | None = None,
    build_dynasty_overview: Callable[[], dict] | None = None,
    build_dynasty_detail: Callable[[], dict] | None = None,
    build_avatar_overview: Callable[[], dict] | None = None,
    build_saves: Callable[[], dict] | None = None,
    build_detail: Callable[..., dict] | None = None,
    build_deceased_list: Callable[[], dict] | None = None,
    build_roleplay_session: Callable[[], dict] | None = None,
    build_world_secret_meta: Callable[[], dict] | None = None,
    build_world_secret_overview: Callable[[], dict] | None = None,
) -> APIRouter:
    router = APIRouter()

    @router.get("/api/v1/query/runtime/status")
    def get_runtime_status_v1():
        if query_service is not None:
            return ok_response(query_service.get_runtime_status())
        return ok_response(build_runtime_status())

    @router.get("/api/v1/query/world/state")
    def get_world_state_v1():
        if query_service is not None:
            return ok_response(query_service.get_world_state())
        return ok_response(build_world_state())

    @router.get("/api/v1/query/world/map")
    def get_world_map_v1():
        if query_service is not None:
            return ok_response(query_service.get_world_map())
        return ok_response(build_world_map())

    @router.get("/api/v1/query/world/map-presets")
    def get_world_map_presets_v1(locale: str | None = None):
        if query_service is not None:
            return ok_response(query_service.get_map_presets(locale=locale))
        return ok_response(build_map_presets(locale=locale))

    @router.get("/api/v1/query/system/current-run")
    def get_current_run_v1():
        if query_service is not None:
            return ok_response(query_service.get_current_run())
        return ok_response(build_current_run())

    @router.get("/api/v1/query/events")
    def get_events_v1(
        avatar_id: str = None,
        avatar_id_1: str = None,
        avatar_id_2: str = None,
        sect_id: int = None,
        major_scope: str = Query("all", pattern="^(all|major|minor)$"),
        cursor: str = None,
        limit: int = 100,
    ):
        return ok_response(
            query_service.get_events_page(
                avatar_id=avatar_id,
                avatar_id_1=avatar_id_1,
                avatar_id_2=avatar_id_2,
                sect_id=sect_id,
                major_scope=major_scope,
                cursor=cursor,
                limit=limit,
            )
            if query_service is not None
            else build_events_page(
                avatar_id=avatar_id,
                avatar_id_1=avatar_id_1,
                avatar_id_2=avatar_id_2,
                sect_id=sect_id,
                major_scope=major_scope,
                cursor=cursor,
                limit=limit,
            )
        )

    @router.get("/api/v1/query/rankings")
    def get_rankings_v1():
        if query_service is not None:
            return ok_response(query_service.get_rankings())
        return ok_response(build_rankings())

    @router.get("/api/v1/query/sect-relations")
    def get_sect_relations_v1():
        if query_service is not None:
            return ok_response(query_service.get_sect_relations())
        return ok_response(build_sect_relations())

    @router.get("/api/v1/query/meta/game-data")
    def get_game_data_v1():
        if query_service is not None:
            return ok_response(query_service.get_game_data())
        return ok_response(build_game_data())

    @router.get("/api/v1/query/meta/avatar-adjust-options")
    def get_avatar_adjust_options_v1():
        if query_service is not None:
            return ok_response(query_service.get_avatar_adjust_options())
        return ok_response(build_avatar_adjust_options())

    @router.get("/api/v1/query/meta/avatars")
    def get_avatar_meta_v1():
        if query_service is not None:
            return ok_response(query_service.get_avatar_meta())
        return ok_response(build_avatar_meta())

    @router.get("/api/v1/query/meta/avatar-list")
    def get_avatar_list_v1():
        if query_service is not None:
            return ok_response(query_service.get_avatar_list())
        return ok_response(build_avatar_list())

    @router.get("/api/v1/query/meta/phenomena")
    def get_phenomena_list_v1():
        if query_service is not None:
            return ok_response(query_service.get_phenomena())
        return ok_response(build_phenomena())

    @router.get("/api/v1/query/sects/territories")
    def get_sect_territories_v1():
        if query_service is not None:
            return ok_response(query_service.get_sect_territories())
        return ok_response(build_sect_territories())

    @router.get("/api/v1/query/mortals/overview")
    def get_mortal_overview_v1():
        if query_service is not None:
            return ok_response(query_service.get_mortal_overview())
        return ok_response(build_mortal_overview())

    @router.get("/api/v1/query/dynasty/overview")
    def get_dynasty_overview_v1():
        if query_service is not None:
            return ok_response(query_service.get_dynasty_overview())
        return ok_response(build_dynasty_overview())

    @router.get("/api/v1/query/dynasty/detail")
    def get_dynasty_detail_v1():
        if query_service is not None:
            return ok_response(query_service.get_dynasty_detail())
        return ok_response(build_dynasty_detail())

    @router.get("/api/v1/query/avatars/overview")
    def get_avatar_overview_v1():
        if query_service is not None:
            return ok_response(query_service.get_avatar_overview())
        return ok_response(build_avatar_overview())

    @router.get("/api/v1/query/saves")
    def get_saves_v1():
        if query_service is not None:
            return ok_response(query_service.get_saves())
        return ok_response(build_saves())

    @router.get("/api/v1/query/detail")
    def get_detail_info_v1(
        target_type: str = Query(alias="type"),
        target_id: str = Query(alias="id"),
    ):
        if query_service is not None:
            return ok_response(
                query_service.get_detail(target_type=target_type, target_id=target_id)
            )
        return ok_response(build_detail(target_type=target_type, target_id=target_id))

    @router.get("/api/v1/query/deceased")
    def get_deceased_list_v1():
        if query_service is not None:
            return ok_response(query_service.get_deceased_list())
        return ok_response(build_deceased_list())

    @router.get("/api/v1/query/roleplay/session")
    def get_roleplay_session_v1():
        if query_service is not None:
            return ok_response(query_service.get_roleplay_session())
        return ok_response(build_roleplay_session())

    @router.get("/api/v1/query/meta/world-secrets")
    def get_world_secret_meta_v1():
        if query_service is not None:
            return ok_response(query_service.get_world_secret_meta())
        return ok_response(build_world_secret_meta())

    @router.get("/api/v1/query/world-secrets/overview")
    def get_world_secret_overview_v1():
        if query_service is not None:
            return ok_response(query_service.get_world_secret_overview())
        return ok_response(build_world_secret_overview())

    return router
