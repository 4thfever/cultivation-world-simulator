...
Write-Host "=== Publish desktop package to Steam ===" -ForegroundColor Cyan
Write-Host "Tag: $tag"
...
Write-Host "`n>>> Step 3/3: Publishing to Steam..." -ForegroundColor Cyan
$PublishScript = Join-Path $ScriptDir "steam\publish.ps1"
$PublishArgs = @(
...
if ($LASTEXITCODE -ne 0) {
    throw "steam/publish.ps1 failed with exit code $LASTEXITCODE"
}

Write-Host "`n=== Steam publish pipeline completed ===" -ForegroundColor Green
Write-Host "Tag: $tag"
Write-Host "Build version: $BuildVersion"
Write-Host "Content root: $ContentRoot"
